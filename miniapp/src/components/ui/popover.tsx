...
export function PopoverContent({ children, className = "" }: { children: ReactNode; className?: string }) {
  return (
    <PopoverPrimitive.Portal>
      <PopoverPrimitive.Content sideOffset={8} collisionPadding={12} className={cn("z-50 w-[min(280px,calc(100vw-24px))] rounded-md border border-border bg-[#1a1a1a] p-3 text-sm text-foreground shadow-lg outline-none animate-in fade-in-0 zoom-in-95", className)}>
        {children}
